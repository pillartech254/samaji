-- ============================================================
--  Samaji — MIGRATION 14 : Teacher portal login + Payroll v2
--  (loans/advances as deductions, allowance breakdown, P9 data)
--  Safe to run multiple times. Run AFTER setup-modules-13.sql.
--
--  Note on setup-modules-13.sql overlap: that migration added its own,
--  separate teacher-portal plumbing (profiles.teacher_id, teacher_payroll,
--  create_teacher_account(), my_teacher_id()). This migration does NOT
--  touch or drop any of that — if it's already applied in your database
--  it's simply left in place, unused by the app going forward. The
--  Teacher Portal (teacher/index.html) and Payroll module in this repo
--  use the mechanism below instead: teachers.auth_user_id (not
--  profiles.teacher_id) for login linkage, and staff/payslips/
--  staff_deductions (not teacher_payroll) for pay data, because it
--  supports loans/advances, allowance breakdown and P9 — none of which
--  teacher_payroll has. If you'd already wired up "Create Login" /
--  create_teacher_account() in production, teachers created that way
--  won't be recognized by this portal until they also sign up at
--  /teacher/ with the same email (which claims the auth_user_id link
--  below) — see section 5.
-- ============================================================
